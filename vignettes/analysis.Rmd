...
validated = FALSE
mo_clone <- metacoder_obj$clone()
```
```{r man_otu_prop_valid}
...
```
```{r man_otu_prop_t1}
abund_data <- transformer(mo_clone$data$otu_abundance, func = ~./sum(.))
```
```{r man_otu_prop_t2}
abund_data <- abund_data %>% transposer(ids = "otu_id", header_name = "samples", preserved_categories = FALSE) 
```
```{r man_otu_prop3}
abund_data <- abund_data %>% dplyr::select_if(is.numeric)
```
```{r man_otu_prop_filt}
