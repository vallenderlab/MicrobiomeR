...
get_heat_tree_plots <- function(obj, rank_list = NULL) {
  ...
  for (rank in rank_list) {
    ...
    smb_data <- metacoder_object %>% taxa::filter_obs(data = c("statistical_data", "taxa_proportions", "taxa_abundance", "stats_tax_data"), n_supertaxa < rank_level, drop_taxa = TRUE)
