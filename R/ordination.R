...
ordination_plot <- function(obj, method = "PCoA", distance = "wunifrac", title = NULL, save = FALSE, only_data = FALSE) {
  ...
  plot <- phyloseq::plot_ordination(physeq = phyloseq_object, ordination = ord, color = "TreatmentGroup") +
    ...
    ) + ggplot2::stat_ellipse(geom = "polygon", alpha = .2, ggplot2::aes(fill = TreatmentGroup), show.legend = FALSE) + ggplot2::scale_fill_manual(values = c("#3288bd", "#d53e4f"))
  plot <- plot + ggplot2::labs(color = "Treatment Group") + ggplot2::scale_color_manual(values = c("Control" = "#3288bd", "Stressed" = "#d53e4f"))
  if (!is.null(title)) {
