correlation_plot <- function(obj, primary_rank, secondary_rank = TRUE,
  ...
  target_data <- target_data %>% dplyr::mutate(color_wilcox_p_value = MicrobiomeR::vlookup(!! quoted_str, super_taxa_data, secondary_rank, "wilcox_p_value")) %>%
  ...
  limits <- MicrobiomeR::get_plot_limits(target_data$mean_treat1, target_data$mean_treat2)
  ...
  target_data <- dplyr::mutate(target_data, rank_label = MicrobiomeR::vlookup(target_data[[primary_rank]], ss_df, primary_rank, "rank_label"))
  target_data <- dplyr::arrange(target_data, label, wilcox_p_value)
  ss_df <- dplyr::arrange(ss_df, wilcox_p_value)

  # Start ggplot2 workflow
  corr <- ggplot(target_data, ggplot2::aes(x = mean_treat1, y = mean_treat2)) +
    ggplot2::geom_polygon(positions, mapping = ggplot2::aes(x = x, y = y, fill = id), alpha = 0.07, show.legend = FALSE) +
    ggplot2::geom_point(data = ss_df, ggplot2::aes(shape = Abundance), size = 3.2, color = "black", stroke = 2, show.legend = FALSE) +
    ggplot2::geom_point(data = target_data, ggplot2::aes(shape = Abundance, color = forcats::fct_reorder(target_data[[secondary_rank]], color_wilcox_p_value, min)), size = 2.5, stroke = 1.5) +
    ...
    ggplot2::scale_y_log10(limits = limits) + ggplot2::scale_x_log10(limits = limits) +
