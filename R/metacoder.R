get_metacoder_obj <- function(obj, ...) {
  # Create a metacoder object from a phyloseq/metacoder/RData file
  obj <- object_handler(obj = obj)
  if (!is.null(format)){
    if (format == "custom_format") {
      # TODO-ROB:  Add a stats parameter to fmt_mc_obj().
      # TODO-ROB: ADD cols parameter where necessary
      metacoder_object <- format_metacoder_object(obj = obj, ...)
    } else {
      metacoder_object <- as_MicrobiomeR_format(obj = obj, ...)
    }
  } else {
    stop("The metacoder object must be validated and formated.  Please provide one of the desired formats.")
  }
}
